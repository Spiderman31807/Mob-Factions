...
public class Faction {
	...
	public static Faction load(CompoundTag compound) {
		UUID factionUUID = compound.hasUUID("UUID") ? compound.getUUID("UUID") : Mth.createInsecureUUID();
		String id = compound.contains("Id") ? compound.getString("Id") : "MISSING-ID";
		final Faction faction =  new Faction(factionUUID, id);
		
		if (compound.contains("Entities")) {
			ListTag list = compound.getList("Entities", 8);
			for (int idx = 0; idx < list.size(); idx++) {
				EntityType.byString(list.getString(idx)).ifPresent((type) -> faction.entities.add(type));
			}
		}
		if (compound.contains("AlliedEntities")) {
			ListTag list = compound.getList("AlliedEntities", 8);
			for (int idx = 0; idx < list.size(); idx++) {
				EntityType.byString(list.getString(idx)).ifPresent((type) -> faction.alliedEntities.add(type));
			}
		}
		if (compound.contains("HostileEntities")) {
			ListTag list = compound.getList("HostileEntities", 8);
			for (int idx = 0; idx < list.size(); idx++) {
				EntityType.byString(list.getString(idx)).ifPresent((type) -> faction.hostileEntities.add(type));
			}
		}
		if (compound.contains("Allies")) {
			ListTag list = compound.getList("Allies", 11);
			for (int idx = 0; idx < list.size(); idx++) {
				try {
					UUID uuid = NbtUtils.loadUUID(list.get(idx));
		...
		if (compound.contains("Enemies")) {
			ListTag list = compound.getList("Enemies", 11);
			for (int idx = 0; idx < list.size(); idx++) {
				try {
					UUID uuid = NbtUtils.loadUUID(list.get(idx));
	...
	public CompoundTag save() {
		...
		this.allied.forEach((uuid) -> alliesList.addTag(alliesList.size(), NbtUtils.createUUID(uuid)));
		compound.put("Allies", alliesList);
		
		final ListTag enemyList = new ListTag();
		this.enemies.forEach((uuid) -> enemyList.addTag(enemyList.size(), NbtUtils.createUUID(uuid)));

...
public class Events {
	@SubscribeEvent
	public static void construstor(EntityEvent.EntityConstructing event) {
	...
	public static void addGoals(Mob mob) {
		...
		Predicate<LivingEntity> filter = (entity) -> mob instanceof FactionHolder holder && holder.isFactionEnemy(entity);
	...
	public static void targeting(LivingChangeTargetEvent event) {
		if (event.getNewAboutToBeSetTarget() instanceof LivingEntity newTarget && event.getEntity().isAlliedTo(newTarget))
